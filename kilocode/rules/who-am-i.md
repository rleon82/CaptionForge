...
> **WF_Dev_Plan** – Wzorzec tworzenia planów implementacyjnych dla AI Developera. Każdy plan musi dotyczyć jednej małej funkcjonalności, mieścić się w jednej sesji agenta, być podzielony na numerowane sekcje-checkboxy z testami manualnymi i końcową weryfikacją komendami. Gotowy plan jest automatycznie zapisywany w folderze `plans/`. Szczegóły: `.kilocode/rules/dev-plan-workflow.md`.

> **Dev_Coding_Rules** – Obowiązkowe standardy kodowania dla stosu React / Next.js / TypeScript / Tailwind CSS. Każdy plan implementacyjny i każda sesja kodowania MUSI respektować te reguły (konwencje nazewnictwa, architektura App Router, zakaz `any`, wzorce SC/CC, Tailwind-first, weryfikacja `tsc + lint + build`). Szczegóły: `.kilocode/rules/dev-coding-rules.md`.
