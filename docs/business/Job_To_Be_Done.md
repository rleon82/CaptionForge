...
```mermaid
graph LR
    subgraph Kasia - Content Creator
        K1[1 konto, 1-2 platformy]
        K2[1 ton głosu per platforma]
        K3[5-8 opisów/tydzień]
        K4[Priorytet: inspiracja + szybkość]
    end

    subgraph Tomek - Freelancer
        T1[6+ kont klientów]
        T2[Różne tony per klient]
        T3[15-50 opisów/tydzień]
        T4[Priorytet: skalowanie + jakość]
    end

    K1 -.->|Mniej złożoności| T1
    K3 -.->|x3-6 wolumen| T3
```
